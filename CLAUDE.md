...
| `CLAUDE.md` | 아키텍처, 워크플로우, 스키마, 구현 원칙 |
| `.claude/agents/*.md` | 에이전트별 작업 기준 |
...
| 에이전트 | 역할 |
|----------|------|
| `scanner-agent` | scan.py 실행, preflight, raw_results.json 생성 |
| `analyst-orchestrator` | 분석 목적에 따라 하위 analyst로 분배 |
| `header-ssl-analyst` | 헤더, 쿠키, CSP, TLS 결과 해석 |
| `network-analyst` | 네트워크 노출, Shodan, 외부 공격면 분석 |
| `code-analyst` | semgrep, dependency, secrets 해석 |
| `owasp-analyst` | OWASP Top 10 분류 및 웹 취약점 해석 |
| `qa-reporter` | filtered_results.json + test_cases.md / report_dev.md 생성 |
| `jira-agent` | Jira create-or-update, dedup label, delta 코멘트 |
...
`analyst-orchestrator`가 분석 목적에 따라 병렬 에이전트 조합을 선택한다.

---

## 4. Full Runtime Workflow
...
| 모드 | 입력 | 스캐너 |
|------|------|--------|
| `--url` | 웹 URL | headers · db · server · network · ssl_labs · shodan · nuclei · zap |
| `--path` | 소스코드 디렉터리 | semgrep · dependency · secrets |
| `--war` | WAR 파일 / WEB-INF 디렉터리 | grype(CVE) · webxml(보안설정) |
...
```bash
python scan.py --url https://target.com                          # URL 전체 스캔
python scan.py --url https://target.com --skip-zap               # ZAP/nuclei 제외
python scan.py --url https://target.com --skip-ai                # AI filtering 제외
python scan.py --path ./my-app                                   # 로컬 코드 스캔
python scan.py --war ./app.war                                   # WAR 파일 SCA 스캔
python scan.py --war ./WEB-INF                                   # WEB-INF 디렉터리 SCA 스캔

python scan.py --from-filtered ./output/report/<ts>/filtered_results.json             # 리포트 재생성
python scan.py --from-filtered ./output/report/<ts>/filtered_results.json --skip-jira # Jira 제외

python scan.py --url https://target.com --baseline ./output/report/<prev>/filtered_results.json          # Regression
python scan.py --from-filtered ./output/report/<ts>/filtered_results.json --baseline ./output/report/<prev>/filtered_results.json
```
...
**Fallback:** `--skip-ai` 또는 AI 실패 시 파이프라인 중단 없이 fallback triage로 진행. API key 미설정 시에는 Claude Code 내부 분석 또는 fallback triage 중 사용자가 선택한다 (Step 0.5 참조).
...
## 10. Interactive Workflow (Claude 행동 규칙)
